...
- `repo-skills` skill: Signals match repo names (root basename or `origin` remote) first; Setup `config:` fragments are deep-merged (objects recursive, arrays unioned, jq recipe) into the project's `.claude/settings.local.json`; new `setup-only` action when a skill is installed but its config is missing; verify checks config keys; `manual:` items reported as follow-ups.
- `REPO-SKILLS.md`: new optional `Setup` field (`config:` / `manual:` sub-bullets). Added project-scope entries for `krane-solutions-business-orchestration`: `hormozi-offer`, `pricing-strategy`, `business-model`, `objection-destroyer` (alexsmedile/hormozi-skills @25ec2b0) and `marketing-council` (coreyhaines31/marketingskills @5b2c000, manual-only via `skillOverrides` `user-invocable-only`). All five re-vetted by skill-scout + full council with SkillSpector 2.12.0 and full manual reads; each carries a 2026-10-16 usage review.
- SkillSpector is now expected on PATH for skill-scout's security gate (installed outside this repo).
